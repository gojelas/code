...
#indicatrice_It(Ic,a_m)
### Modèle PLAT+TERME de température mais avec le même coeficient par tranche de 5ans
########################
#### Model function ####
########################
model_function=function(dxt,ext,wxt,Ic,a,c_x,a_c,ages,years,x1,x2)
{
...
### compute model
get_estimation=function(data, ages, years, c, a_c=65, x1, x2)
{
...
get_predict=function(estim,years_pred,i,kt5_order=c(2,0,0))
{
  "
  ...
  fit_gc=Arima(estim$g,order = c(1,1,0))
...
#forecasting fonction
get_predict_f=function(estim,years_pred,i,kt5_order=c(2,0,1),val=FALSE)
{
  "
  i indique le choix du scénario RCP, 1 pour RCP2.6, 2 pour RCP4.5 et 3 pour RCP8.5
  "
...
get_best_params=function(data,c,x1,x2,ages,years,Ic)
{
  ...
  for (i in 1:length(x1))
  {
    ...
    {
      for (x in 1:length(c)){
        est=get_estimation(data,ages,years1,c[x],a_c = 65,x1[i],x2[j])
        a_m=est$a_m
        Ic=df[(years1[1]-1961+1):(tail(years1,1)-1961+1),6]
        ...
        model=model_function(fr_m,fr_e,wei,Ic,a_m,c[x],a_c,ages,years,x1[i],x2[j])
        mu_pred=get_predict(est,years_pred = years2)
...
get_best_params_2=function(data,c,x1,x2,ages,years,Ic)
{
  #initialize parameters
  c_0=c()
  #x1_0=0
  ...
  for (x in 1:length(c)){
    est=get_estimation(data,ages,years1,c[x],a_c = 65,x1,x2)
    a_m=est$a_m
    model=model_function(fr_m,fr_e,wei,Ic,a_m,c[x],a_c,ages,years,x1,x2)
    mu_pred=get_predict(est,years_pred = years2)
    m2=mean(abs((mu_pred$mxt_pred-mu_obs)/mu_obs))
    m1=mape(model,ages,years1)
    mape2=rbind(mape2,m2)
  ...
  #  })
  perf=data.frame(c_0,mape1,mape2,row.names = 1)
  colnames(perf)=c('mape_train','mape_test')
  list(
    c=c_0,
    #x1=x1_0,
    #x2=x2_0,
...
sim_ic_method=function(mode, ages.fit, years.fit, 
                       years.pred, x1, x2, c, a_c=65, N=100)
{
  ...
  kt5_est=ts(l$y,start=1,frequency = 1)
  fit5=Arima(kt5_est,order = c(2,0,0))
  #fit5=auto.arima(kt5)
...
sim_ic_method2=function(model, ages.fit, years.fit, years.pred, x1, x2, c, 
                        a_c=65, N=100,kt5_order=c(2,0,0),val=TRUE)
{
  ...
  ### Gamma
  #fit_gc=Arima(model$gc,order = c(1,0,0),include.mean = FALSE)
  fit_gc=Arima(model$gc,order = c(1,1,0))
  pc=forecast(fit_gc,h=150)
  sim_gc=ts(replicate(N,simulate(fit_gc,nsim = 150)),start = 1951)
  gc_pred=pc$mean
  #gc_pred=sim_gc
  ...
  for(i in 1:N)
  {
    ...
    teta=c(ax,sim_kt[1,,i],sim_kt[2,,i],sim_kt[3,,i],sim_kt[4,,i],sim_kt5,gcc)
    x=ages.fit
    x.bar=mean(x)
    n=n.a*n.t
    n.c=n.a+n.t-1
